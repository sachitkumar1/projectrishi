...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* Member Dashboard — describes the app + its Google permissions.
          This section exists to satisfy Google's OAuth "App Homepage"
          requirements: it names the app, describes what it does, and explains
          transparently why we request Google Calendar and Gmail access. It must
          stay publicly visible (no login) and keep the privacy-policy link. */}
      <section className="border-t border-pine/10 bg-pine/[0.03]">
        <div className="container-rishi py-20">
          <Reveal>
            <SectionHeading
              eyebrow="For Members"
              title="The Project RISHI Member Dashboard"
            />
          </Reveal>
          <div className="mt-8 grid gap-10 lg:grid-cols-2">
            <Reveal delay={0.1}>
              <p className="text-ink/75">
                Members of our chapter sign in to a private dashboard we built to
                run the club. It&apos;s where we assign and track project tasks,
                keep a shared calendar of meetings and events, message each other,
                look up members&apos; contact details, and send club
                announcements, newsletters, and reminders. Access is limited to
                people on our club roster.
              </p>
              <p className="mt-4 text-ink/75">
                Signing in uses Google only to confirm your name and email
                address. Two further features are optional, off by default, and
                connected by the member themselves:
              </p>
              <Link
                href="/login"
                className="btn mt-7 bg-pine text-paper hover:-translate-y-0.5"
              >
                Member login
              </Link>
            </Reveal>
            ...
            <Reveal delay={0.15}>
              <div className="space-y-5">
                <div className="rounded-2xl border border-pine/15 bg-paper p-6">
                  <h3 className="font-display text-lg font-semibold text-pine-deep">
                    Google Calendar
                  </h3>
                  <p className="mt-2 text-sm text-ink/70">
                    If a member connects their calendar, we add the tasks and
                    events assigned to them in the dashboard to their own Google
                    Calendar, so club commitments show up alongside everything
                    else. The sync is one-way: we only create and update the
                    events our app added, and we never read, list, or import a
                    member&apos;s existing calendar events. Disconnecting stops
                    the sync and removes the events we created.
                  </p>
                </div>
                <div className="rounded-2xl border border-pine/15 bg-paper p-6">
                  <h3 className="font-display text-lg font-semibold text-pine-deep">
                  ...
                  <p className="mt-2 text-sm text-ink/70">
                    An officer connects the club&apos;s Google account so the
                    dashboard can send email on the club&apos;s behalf: task and
                    event notifications, due-date reminders, and the
                    announcements and newsletters our officers write. This is
                    send-only access &mdash; it cannot read, view, modify, or
                    delete any mail in the account.
                  </p>
                </div>
                <p className="text-sm text-ink/55">
                  We never sell member data or use it for advertising. Read our{" "}
